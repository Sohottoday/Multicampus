...
SimplePos22("대한민국의 영토는 한반도와 그 부속도서로 한다")   # 교재 182p에 용도 설명
...
wordcloud(words$keyword, words$freq, 
          min.freq = 2, 
          random.order = FALSE, 
          rot.per = 0.3, scale = c(4, 1),  # scale 출력되는 폰트의 크기.
          colors = rainbow(7))                # rot.per 는 가로로 기울어진 텍스트의 비율, 1일수록 많아진다.
...
wordcloud(words$keyword, words$freq,family="lett") # 값이 클수록 크게 나오고 적을수록 작게 나온다.
install.packages("htmlwidgets")    # html로 저장할 때 도와주는 패키지.
library(htmlwidgets)        
saveWidget(result,"tmpwc.html",selfcontained = F)   #selfcontained = F는 css와 js파일을 별도 폴더에 저장해줌.
head(demoFreq)
str(demoFreq)
# wordcloud2는 웹에서 쓰는 용도가 아니라면 자주쓰이지는 않는다.
