...
mk <- geocode("seoul", source = "google") # source 명령어는 생략 가능.
print(mk)
cen <- c(mk$lon, mk$lat)
map <- get_googlemap(center=cen, maptype="roadmap",zoom=11, marker=mk)
ggmap(map)
mk <- geocode(enc2utf8("부산"), source = "google") # 한글로 바아올때에는 enc2utf8로 받아오기.
cen <- c(mk$lon, mk$lat)
map <- get_googlemap(center=cen, maptype="roadmap",zoom=11, marker=mk)
ggmap(map)
...
ggmap(map) + 
  geom_point(aes(x=mk$lon, y=mk$lat), alpha=0.4, size=5, color="pink") + #alpha는 투명도.
  geom_text(aes(x=mk$lon, y=mk$lat, label="우리가 공부하는 곳", vjust=0, hjust=0)) #텍스트를 살짝 거리를 벌려주고 싶을때 vjust, hjust 사용.
...
df <- data.frame(name=names,
                 lon=gc$lon,
                 lat=gc$lat)
cen <- c(mean(df$lon),mean(df$lat)) # 한 화면에 마커를 다 보이기 위해 중앙값 설정.
...
# 다음 행은 한번만 수행시키셩
station_lonlat = mutate_geocode(station_df, station_list, source = 'google') # 데이터프레임에 변수를 추가해줌.
station_lonlat
...
map3<-leaflet() %>% addTiles() %>%  addPopups(126.97797, 37.56654, content1, options = popupOptions()) # options = popupOptions() => 팝업 계속 띄워두는 효과. 대신 끄면 못킴.
map3
...
map3 # setView가 없는 대신 addPopups를 기준으로 중심값으로 센터값 설정.
...
leaflet(wifi_data) %>% 
  ...
  addProviderTiles('Stamen.Toner') %>%   #addTiles()를 주면 기본적으로 roadmap형식이고 이런식으로 다른 형식을 설정 가능.
  addCircles(lng = ~lon, lat = ~lat)
