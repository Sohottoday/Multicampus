...
#devtools::install_github("cardiomoon/Kormaps")   // 현재 설치가 안되는 상태.
library(Kormaps)
library(dplyr)
library(leaflet)
...
map4    #stroke=TRUE를 주면 주변에 선을 준다.
# smoothFactor 은 변화시켜도 큰 의미가 없다.
...
k3@data[grep('^11150', k3@data$C행정구역별_읍면동),  # value값을 주지 않았으므로 index만 알려줌.
        c("C행정구역별_읍면동", "행정구역별_읍면동")]
...
mypopup <- paste0(mymap$name,'<br> 1인가구: ',k3@data$'일인가구') # 여긴 왜 ''를 주는가?
...
ret1
ret2 <- dbFetch(result3, 2) # 갯수를 주지 않으면 그 당시까지 읽은 순서 이후를 모두 읽어옴.
ret2
...
dbRemoveTable(conn,"cars") # 테이블 지우는 함수.
dbGetQuery(conn, "SELECT * FROM cars")
# 방법 2
# 이미 만들어진 테이블에 값만 넣고 싶을때 사용=>(dbSendUpdate).
...
dbSendUpdate(conn,"INSERT INTO NEWS VALUES(news_seq.nextval, '유니코', 'R로 데이터를 입력하는거 테스트입니당..', sysdate, 0,'테스트')")
dbGetQuery(conn, "SELECT * FROM NEWS") # 위에 왜 안넣어지는가?
# 데이터 수정
