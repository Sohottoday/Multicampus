...
Sys.setlocale("LC_ALL", "English")    # 이 함수를 통해 영어환경으로 변환환
games_table <- readHTMLTable(elem_html, header = T, stringsAsFactors = FALSE, )[[1]]
Sys.setlocale() # 다시 한국환경으로 변환
Encoding(names(games_table)) <- "UTF-8" # 그래도 제목은 깨지므로 제목도 변환해줌.
head(games_table)
str(games_table)
View(games_table)
tail(games_table)
...
imsi + theme_classic()
imsi + theme_void()
# ggplot 테마를 더 추가할 수 있는 패키지가 존재한다.
...
ggplot(data=w, aes(x=year, y=weight)) + geom_bar() # 갯수를 세서 막대를 그리는게 기본이라..2014년도 1개 2015년도 1개 이런식으로 해석하게 됨.
...
ggsave("ggplot_test.png")    # ggplot이 들고있는 저장 함수.
...
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+
  geom_point()+guides(color = guide_legend(nrow = 6))   # guides 함수 =>범례라는 의미.
